...
namespace StatsDirect.TemplateProcessing
{
    public sealed class ScriptEngine : IScriptEngine
    {
        ...
        private static object RunR(ITemplateHost host, string code, ParameterBag parameters)
        {
            ...
            if (0 != exitCode)
                throw new Exception(RController.GetErrorText(host) ?? "R did not complete successfully and did not save an error message");
            else
            {
                ParameterBag pb = RController.FilesToParameterBag(host);
