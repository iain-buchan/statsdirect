...
namespace StatsDirect.TemplateProcessing
{
    public class CreoleRtfReportRenderer : ReportRenderer
    {
        ...
        public override string Render(/* TODO: IPreferences */ ITemplateHost host, string template, ParameterBag substitutions)
        {
            ...
            string templateWithInclusions = ResolveTemplates(host, protectedTemplate, new List<string>());
        ...
        private static string ResolveTemplates(IReportHost host, string rawCreole, ICollection<string> knownInclusions)
        {
            StringBuilder sb = new();
            int sourcePosition = 0;
            do
            {
                ...
                if (!knownInclusions.Contains(includedName))
                {
                    ...
                    string includedContent = host.GetReportTemplate(includedName);
                    if (!string.IsNullOrEmpty(includedContent))
                        sb.Append(ResolveTemplates(host, includedContent, knownInclusions));
