...
namespace StatsDirect.TemplateProcessing
{
    ...
    public sealed class TemplateProcessor : ITemplateProcessor
    {
        ...
        public const string STATSDIRECT_FRAME_PANE = "statsdirect-frame-pane"; // Used by host, though the logic probably should be moved entirely to one or the other.
        ...
        {
            ChartDefinition definition = new(host.ChartPreferences) { ChartType = step.ChartType };
            // Series: First X...
        ...
        public StepOutput ExecuteInternal(OutputFrameStep step, ParameterBag parameters)
        {
            ...
            {
                host.OutputFrame(frame, step.KeepSelection, step.IsFormulae, step.MissingIndicator, parameters, step.DefaultPlacement);
            }
            return StepOutput.Empty();
        }
        ...
        {
            string content = reportStep.Text ?? host.GetReportTemplate(reportStep.FileName);
            ReportTemplateAndParameters filledTemplate = new(new ReportTemplate(content, reportStep.MimeType), parameters);

            ParameterBag outputParameters = host.OutputReport(filledTemplate, reportStep.Operation, parameters);
