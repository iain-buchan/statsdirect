...
namespace StatsDirect.Charting
{
    public static class ChartOptionProcessor
    {
        public static ChartOptions PreprocessChartOptions(ChartStep step, ParameterBag parameters, ChartDefinition definition, string dataName, IPreferencesHost preferences)
        {
            ...
            switch (step.ChartType)
            {
                case ChartType.AgreementPair:
                    return PreprocessAgreementOptions(definition, step, parameters);
                case ChartType.Bar:
                ...
                case ChartType.ErrorBar:
                    return PreprocessErrorBarOptions(definition, parameters, dataName);
                case ChartType.Forest:
                    return PreprocessForestOptions(definition, parameters, dataName);
                case ChartType.Gini:
                ...
                case ChartType.Normal:
                    return PreprocessNormalOptions(definition, parameters, dataName);
                case ChartType.Pyramid:
                    return PreprocessPyramidOptions(definition, parameters, dataName);
                case ChartType.ScatterXY:
                    return PreprocessScatterXYOptions(step, definition, dataName);
                case ChartType.ROC:
                    return PreprocessRocOptions(preferences, parameters, definition);
                case ChartType.Spread:
                    return PreprocessSpreadOptions(definition, dataName);
                case ChartType.Survival:
                    return PreprocessSurvivalOptions(definition, parameters, dataName);
                default:
        ...
        private static SurvivalOptions PreprocessSurvivalOptions(ChartDefinition definition, ParameterBag parameters, string dataName)
        {
            SurvivalOptions survivalOptions = new(definition.ChartPreferences)
            {
                ShouldAutoscale =
                    !definition.ChartPreferences.RequestScaleLimits,
                Title =
                    null == dataName
                        ? "Survival plot"
        ...
        private static SpreadOptions PreprocessSpreadOptions(ChartDefinition definition, string dataName)
        {
            ...
            SpreadOptions spreadOptions = new(definition.ChartPreferences)
            {
                ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits,
                Title =
                    null == dataName
                        ? "Spread plot"
        ...
        private static ROCOptions PreprocessRocOptions(IPreferencesHost host, ParameterBag parameters, ChartDefinition definition)
        {
            ...
            ROCOptions rocOptions = new(definition.ChartPreferences, definition.XSeries)
            {
                ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits,
                Title = null == dataName
                    ? "ROC plot"
        ...
        {
            ScatterXYOptions sOptions = new(definition.ChartPreferences, definition.XSeries, false)
            {
        ...
        private static PyramidOptions PreprocessPyramidOptions(ChartDefinition definition, ParameterBag parameters, string dataName)
        {
            PyramidOptions pOptions = new(definition.ChartPreferences) { ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits };
        ...
        private static NormalOptions PreprocessNormalOptions(ChartDefinition definition, ParameterBag parameters, string dataName)
        {
            NormalOptions nOptions = new(definition.ChartPreferences)
            {
                ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits,
        ...
        {
            ScatterXYOptions sOptions = new(definition.ChartPreferences, definition.XSeries, true)
            {
                ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits,
                Title =
                    null == dataName
        ...
        private static LadderOptions PreprocessLadderOptions(ChartDefinition definition, string dataName)
        {
            ...
            LadderOptions ladderOptions = new(definition.ChartPreferences)
            {
                ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits,
                Title =
                    null == dataName
        ...
        private static HistogramOptions PreprocessHistogramOptions(ChartStep step, ChartDefinition definition)
        {
            ...
            HistogramOptions hOptions = new(definition.ChartPreferences)
            {
                IsAscii = step.IsAscii,
        ...
        {
            GiniOptions giniOptions = new(definition.ChartPreferences)
            {
                ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits,
                Title =
                    null == dataName
        ...
        private static ForestOptions PreprocessForestOptions(ChartDefinition definition, ParameterBag parameters, string dataName)
        {
            ForestOptions fOptions = new(definition.ChartPreferences)
            {
                cco = 0.95,
                ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits,
                Title =
        ...
        private static ErrorBarOptions PreprocessErrorBarOptions(ChartDefinition definition, ParameterBag parameters, string dataName)
        {
            if (!parameters.ContainsKey("xdat"))
                throw new ArgumentException("Chart expected parameter \"xdat\", which was not supplied");
            if (!parameters.ContainsKey("ydat"))
            ...
            ErrorBarOptions errorBarOptions = new(definition.ChartPreferences)
            {
                ShouldAutoscale =
                    !definition.ChartPreferences.RequestScaleLimits,
                Title = null == dataName
        ...
        {
            ControlOptions controlOptions = new(definition.ChartPreferences)
            {
                ShouldBoxAxes = definition.ChartPreferences.BoxAxes,
                ShouldAutoscale =
                    !definition.ChartPreferences.RequestScaleLimits,
        ...
        {
            BoxWhiskerOptions bwOptions = new(definition.ChartPreferences)
            {
                Title = null == dataName
                            ? "Box & whisker plot"
        ...
        {
            BarOptions barOptions = new(definition.ChartPreferences)
            {
                ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits,
                Title =
                    null == dataName
        ...
        private static AgreementOptions PreprocessAgreementOptions(ChartDefinition definition, ChartStep step, ParameterBag parameters)
        {
            AgreementOptions aOptions = new(definition.ChartPreferences)
            {
                ShouldAutoscale = !definition.ChartPreferences.RequestScaleLimits,
                Title = step.ChartTitle,
