    ...
    class CoxSurvivalOrHazardChartRenderer : AbstractChartRenderer, IChartRenderer
    {
        ...
        ParameterBag IChartRenderer.Plot(ITemplateHost host, bool isForReturnedParametersOnly)
        {
            ...
            if (options.grouped)
            {
                ...
                {
                    MarkerType legendMarker = host.ChartPreferences.MarkerTypes[(k - 1) % 9].Clone();
            ...
            MarkerType mt = host.ChartPreferences.MarkerTypes[igp % 9];
            ...
            for (int i = 1; i <= options.iobs; i++)
            {
                if (i > 1 && (options.grouped ? options.z[i].Id != options.z[i - 1].Id : options.z[i].Stratum != options.z[i - 1].Stratum))
                {
                    ...
                    ix1 = ix0;
                    iy1 = iy0;
                    mt = host.ChartPreferences.MarkerTypes[igp % 9];
