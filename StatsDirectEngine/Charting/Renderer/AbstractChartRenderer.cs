...
{
    /// <summary>
    /// A convenient holder for common renderer functionality that has got out of hand.
    /// </summary>
    /// <remarks>TODO: Should not be public, but ctlSeriesOptions in the Windows UI uses a static function from here to allocate markers.</remarks>
    public abstract class AbstractChartRenderer : IDisposable
    {
        ...
        {
            //  Drawing objects
            AxisLabelFontDescriptor = Definition.ChartPreferences.AxisLabelFont;
            AxisTitleFontDescriptor = Definition.ChartPreferences.AxisTitleFont;
            LabelFontDescriptor = Definition.ChartPreferences.LabelFont;
            LegendFontDescriptor = Definition.ChartPreferences.LegendFont;
            TitleFontDescriptor = Definition.ChartPreferences.TitleFont;
        ...
        protected void AssignMarkersToSeries(IList<ISeries> s)
        {
            for (int i = 0; i < s.Count; i++)
            {
                ...
                SetSeriesFromMarkerTypeAndOptions(ds, Definition.ChartPreferences.MarkerTypes[mkr], null);
            }
        }
        ...
        protected void AssignMarkersToSeries(IList<ISeries> s, GenericOptions opts)
        {
            if (opts?.MarkerTypes == null || opts.MarkerTypes.Count < 1)
            {
                for (int i = 0; i < s.Count; i++)
                {
                    DoubleSeries ds = (DoubleSeries)s[i];
                    int mkr = ChartOptions.SeriesNumberToMarkerNumber(i);
                    SetSeriesFromMarkerTypeAndOptions(ds, Definition.ChartPreferences.MarkerTypes[mkr], opts);
                }
            }
            else
            {
        ...
        protected IList<MarkerType> MarkersFromDescriptors(IList<SeriesOptionsDescriptor> seriesOptionsDescriptors, bool shouldForceIsFilled, bool forcedIsFilled, bool shouldForceFillStyle, FillStyle forcedFillStyle)
            => MarkersFromPreferencesAndDescriptors(Definition.ChartPreferences.MarkerTypes, seriesOptionsDescriptors, shouldForceIsFilled, forcedIsFilled, shouldForceFillStyle, forcedFillStyle);

        /// <remarks>Public as this is needed by ctlChartOptions in the Windows UI, which it really shouldn't be.</remarks>
        public static IList<MarkerType> MarkersFromPreferencesAndDescriptors(IReadOnlyList<MarkerType> preferredMarkerTypes, IList<SeriesOptionsDescriptor> seriesOptionsDescriptors, bool shouldForceIsFilled, bool forcedIsFilled, bool shouldForceFillStyle, FillStyle forcedFillStyle)
        {
            List<MarkerType> markerTypes = new(seriesOptionsDescriptors.Count);
            foreach (SeriesOptionsDescriptor t in seriesOptionsDescriptors)
            {
                ...
                MarkerType clone = preferredMarkerTypes[mkr].Clone();
