...
namespace StatsDirect.Charting.Renderer
{
    class AgreementPairChartRenderer : AbstractXYChartRenderer, IChartRenderer
    {
        ...
        ParameterBag IChartRenderer.Plot(/* TODO: IPreferences*/ ITemplateHost _, bool isForReturnedParametersOnly)
        {
            ...
            PenDescriptor p = GetMarkerPen(Definition.ChartPreferences.MarkerTypes[0]);
            if (aOptions.HasLimits)
            {
                if (aOptions.lla < mxdMin)
                    mxdMin = aOptions.lla;
                ...
                if (string.IsNullOrEmpty(ytxt))
                    ytxt = "difference";
                axisScales = PlotXYInternal(aOptions.av, aOptions.mxd, xtxt, ytxt, "Agreement Plot (" + Formatting.XRound(100 * (1 - aOptions.P0), 2) + "% limits of agreement)", false, DataMinMax.XCalc_YPreset, Definition.ChartPreferences.MarkerTypes[0].MarkerSize, Definition.ChartPreferences.MarkerTypes[0].MarkerShape, Definition.ChartPreferences.MarkerTypes[0].IsMarkerFilled, p, false, ChartAreaShape.Default, 0, 0, mxdMin, mxdMax);
            }
            else
            {
                string xtxt = Definition.ChartOptions.XAxisTitle;
                ...
                axisScales = PlotXYInternal(aOptions.av, aOptions.mxd, xtxt, ytxt, "Agreement Plot", false, DataMinMax.XCalc_YPreset, Definition.ChartPreferences.MarkerTypes[0].MarkerSize, Definition.ChartPreferences.MarkerTypes[0].MarkerShape, Definition.ChartPreferences.MarkerTypes[0].IsMarkerFilled, p, false, ChartAreaShape.Default, 0, 0, mxdMin, mxdMax);
            }
