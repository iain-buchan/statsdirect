    ...
    public static class ImportExport
    {
        public static StepOutput FileImportWorksheet(ITemplateHost host, ParameterBag parameters)
        {
            ...
            OpenFileDialogOptions options = new()
            {
                ...
                Filter = "Comma delimited (*.csv)|*.csv|Tab delimited (*.tab)|*.tab|Text file (*.txt)|*.txt|All files (*.*)|*.*",
                ShowHelp = true,
                Title = "Import worksheet data"
            };
            FileDialogResult result = host.RequestFile(options);

            if (DialogResult.OK != result.DialogResult)
                return null;

            // If it's an Excel file, load it instead
            string suffix = Path.GetExtension(result.FileName);
            ...
            return FileImportAscii(result.FileName);
        }


        public static StepOutput FileImportReport(IUserInterface host)
        {
            ...
            using StreamReader sr = File.OpenText(result.FileName);
        ...
        public static StepOutput FileExportWorksheet(ITemplateHost host, ParameterBag parameters)
        {
            ...
            SaveFileDialogOptions options = new()
            {
                FileName = source.Contains('.') ? source[..^4] + ".csv" : source + ".csv",
            ...
            if (DialogResult.OK == result.DialogResult)
            {
                bool useTabDelimiter = result.FileName[^3..].ToLower(CultureInfo.InvariantCulture) == "tab";
