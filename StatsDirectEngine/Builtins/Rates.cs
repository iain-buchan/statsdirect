...
namespace StatsDirect.Builtins
{
    public static class Rates
    {
        ...
        public static StepOutput RptStdrr(IChartPreferencesHost host, ParameterBag parameters)
        {
            double cco = parameters["cco"].AsDouble;
            if (cco <= 0)
                cco = 0.95;
            ...
            chartParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Correlation, new CorrelationOptions(host.ChartPreferences, k + 2, title, rkr, rkrl, rkru, rkw, pg, "Stratified rate ratio plot (direct standardization)", "rate ratio (" + Formatting.XRound(cco * 100, 1) + "% confidence interval)", Transformation.Log, false)));
