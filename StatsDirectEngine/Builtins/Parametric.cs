...
namespace StatsDirect.Builtins
{
    public static class Parametric
    {
        ...
        public static StepOutput RptNormality(IPreferencesAndProgressBar host, ParameterBag parameters)
        {
            ...
            foreach (IVariable v in frame.Variables)
            {
                ...
                NormalOptions nOptions = new(host.ChartPreferences) { ShouldScaleZ = true, Method = NormalOptions.ScoreMethod.Blom };
                ChartDefinition cd = new(host.ChartPreferences) { ChartOptions = nOptions, ChartType = ChartType.Normal };
        ...
        public static StepOutput RptTPaired(IChartPreferencesHost host, ParameterBag parameters)
        {
            DataFrame Data = parameters["data"].AsDataFrame;
            double GAMMA = parameters["gamma"].AsDouble;
            ...
            if (DoAgree)
            {
                ...
                chartParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Ties, new TiesOptions(host.ChartPreferences, x, y, nx, lla, ula, GAMMA, v0.Title, v1.Title, mean)));
            }
            else
            {
