    ...
    public static class Chi
    {
        ...
        private static StepOutput RptChi2ByN(ParameterBag parameters, Chi2ByNTrend z)
        {
            ...
            if (datFrame.VariableCount < (z == Chi2ByNTrend.WithTrend ? 3 : 2))
                throw new Templates.InvalidDataException("Invalid data: Please fill in the same number of rows in each column without gaps");
            ...
            for (int r = 1; r <= rows; r++)
            {
                ...
                double t1 = a1 + b1;
                if (t1 <= 0.0)
                    throw new Templates.InvalidDataException("Invalid data: row " + r.ToString() + " total is not greater than zero, which it must be for this calculation");
