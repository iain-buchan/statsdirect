...
namespace StatsDirect.Builtins
{
    public static class Analysis
    {
        public static StepOutput RptRateDirectStd(ParameterBag parameters)
        {
            ...
            if (refntot <= 0.0)
                throw new Templates.InvalidDataException("Total reference group size must be greater than zero");
        ...
        public static StepOutput RptMiscLikely(ParameterBag parameters)
        {
            ...
            for (int i = 1; i <= rows; i++)
            {
                if (c1[i] < 0 || c2[i] < 0 || c1[i] == Constant.MISSING || c2[i] == Constant.MISSING)
                    throw new Templates.InvalidDataException("All values must be >= 0");
            ...
            if (c1Tot <= 0 || c2Tot <= 0)
                throw new Templates.InvalidDataException("Total of -feature and total of +feature must both be > 0");
        ...
        public static StepOutput RptMiscRelRisk(ParameterBag parameters)
        {
            ...
            if (a + c <= 0 || b + d <= 0 || b <= 0)
                throw new Templates.InvalidDataException("Relative risk can not be calculated for these data.");
        ...
        public static StepOutput RptPropPairs(ParameterBag parameters)
        {
            ...
            if (n <= 0)
                throw new Templates.InvalidDataException("Total number in study must be at least 1");
            double r = parameters["r"].AsDouble;
            double s = parameters["s"].AsDouble;
            double t = parameters["t"].AsDouble;

            if (n < r + s + t)
                throw new Templates.InvalidDataException("Total number in study must be at least the sum of the number responding in both categories + first category only + second category only");
