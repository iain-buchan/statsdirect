...
namespace StatsDirect.Builtins
{
    public static class Regress
    {
        ...
        public static StepOutput PlotResidualsSimple(IChartPreferencesHost host, ParameterBag parameters)
        {
            ...
            outputParameters.AddOutput("residualsVsY", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, predicted, residual, "Fitted " + context.YTitle, "Residuals (Y - y fit)", "Residuals vs. Fitted Y [linear regression]", true, DataMinMax.XCalc_YCalc)));
            outputParameters.AddOutput("residualsVsPredictor", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, context.X, residual, "Predictor: " + context.XTitle, "Residuals (Y - y fit)", "Residuals vs. Predictor [linear regression]", true, DataMinMax.XCalc_YCalc)));
            ...
            outputParameters.AddOutput("residualsNormalPlot", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, ranked, ranks, "Residual (Y - y fit)", "van der Waerden normal score", "Normal Plot for Residuals [linear regression]", false, DataMinMax.XCalc_YCalc)));
            return new StepOutput(outputParameters);
        }
        ...
        /// <returns></returns>
        public static StepOutput PlotSeCi(IChartPreferencesHost host, ParameterBag parameters)
        {
            SimpleLinearRegressionContext context = GetSimpleLinearRegressionContextWithData(parameters);
            context.CalculateLeastSquaresMethod();
            ...
            ParameterBag outputParameters = new();
            outputParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.LinearRegressionAndMaybeSeCiOrPredictionInterval, new LinearRegressionAndMaybeSeCiOrPredictionIntervalOptions("SE and " + Formatting.XRound((1.0 - context.P0) * 100, 1) + "% CI for regression estimate", context.Slope, context.YIntercept, true, context.XTitle, context.YTitle, context.PERT, context.NX, context.MS, context.SumX, context.SSX, false), new DoubleSeries(context.X, context.XTitle), new DoubleSeries(context.Y, context.YTitle)));
            return new StepOutput(outputParameters);
        }

        public static StepOutput PlotPredictionInterval(IChartPreferencesHost host, ParameterBag parameters)
        {
            SimpleLinearRegressionContext context = GetSimpleLinearRegressionContextWithData(parameters);
            context.CalculateLeastSquaresMethod();
            double REGGAMMA = parameters["gamma"].AsDouble;
            ...
            ParameterBag outputParameters = new();
            outputParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.LinearRegressionAndMaybeSeCiOrPredictionInterval, new LinearRegressionAndMaybeSeCiOrPredictionIntervalOptions(Formatting.XRound((1.0 - context.P0) * 100, 1) + "% Prediction Interval", context.Slope, context.YIntercept, true, context.XTitle, context.YTitle, context.PERT, context.NX, context.MS, context.SumX, context.SSX, true), new DoubleSeries(context.X, context.XTitle), new DoubleSeries(context.Y, context.YTitle)));
            return new StepOutput(outputParameters);
        }
        ...
        public static StepOutput PlotMultipleLinearRegressionResiduals(IChartPreferencesHost host, ParameterBag parameters)
        {
            MultipleLinearRegressionContext context = GetMultipleLinearRegressionContext(parameters);
            double[] r;
            ...
            chartList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, context.FV, context.R, "Fitted Y (y fit)", "Residual (Y - y fit)", "Residuals vs. Fitted Y [linear regression]", true, DataMinMax.XCalc_YCalc)))));
            ...
            for (int i = 1; i <= context.P; i++)
            {
                if (i > 1 || !context.DoC)
                {
                    ...
                    for (int j = 1; j <= context.N; j++)
                        r[j] = context.X[j, i];
                    chartList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, r, context.R, "Predictor: " + context.Titles[i], "Residual (Y - y fit)", "Residuals vs. Predictor " + k.ToString() + " [linear regression]", true, DataMinMax.XCalc_YCalc)))));
                }
            }
            r = new double[context.N + 1];
            ...
            chartList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, context.R, r, "Residual (Y - y fit)", "van der Waerden normal score", "Normal Plot for Residuals (linear regression)", true, DataMinMax.XCalc_YCalc)))));
            return new StepOutput(outputParameters);
        }
        ...
        public static StepOutput RptLinearizedEstimatePlot(IChartPreferencesHost host, ParameterBag parameters)
        {
            DataFrame fY = parameters["y"].AsDataFrame;
            DoubleVariable vY = (DoubleVariable)fY.Variables[0];
            DataFrame fX = parameters["x"].AsDataFrame;
            ...
            ParameterBag outputParameters = new();
            outputParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.LinearizedEstimation, new LinearizedEstimationOptions(host.ChartPreferences, string.Empty, model, context.A, context.G, vX.Title, vY.Title), new DoubleSeries(vX.Data, vX.Title), new DoubleSeries(vY.Data, vY.Title)));
            return new StepOutput(outputParameters);
        }
        ...
        public static StepOutput RptPolynomialRegressionPlot(IChartPreferencesHost host, ParameterBag parameters)
        {
            MultipleLinearRegressionContext context = (MultipleLinearRegressionContext)parameters["context"].AsObject;
            ParameterBag outputParameters = new();
            IList<ParameterBag> chartList = new List<ParameterBag>();
            ...
            for (int mode = 0; mode <= 2; mode++)
            {
                ...
                chartParameters.AddOutput("chart", PlotPoly(host.ChartPreferences, parameters, mode, context.H, context.B, context.SSREG - context.SSY, context.N, context.P));
            }
            return new StepOutput(outputParameters);
        }

        private static IRenderable PlotPoly(IChartPreferences chartPreferences, ParameterBag parameters, int mode, double[,] xtxi, double[] bd, double rss, int nx, int P)
        {
            DataFrame fY = parameters["y"].AsDataFrame;
            DoubleVariable vY = (DoubleVariable)fY.Variables[0];
            DataFrame fX = parameters["x"].AsDataFrame;
            ...
            return ChartRendererFactory.PrepForLater(chartPreferences, ChartType.PolynomialRegression, new PolynomialRegressionOptions(title, mode, xtxi, bd, rss, nx, P, gamma, vX.Title, vY.Title), new DoubleSeries(vX.Data, vX.Title), new DoubleSeries(vY.Data, vY.Title));
        }
        ...
        public static StepOutput PlotLogisticRegressionDiagnostics(IChartPreferencesHost host, ParameterBag parameters)
        {
            MultipleLinearRegressionContext context = (MultipleLinearRegressionContext)parameters["context"].AsObject;
            double[] t = context.T;
            double[] y = context.Y;
            ...
            chartsList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, xx, yy1, ep, db, db + " vs. " + ep, false, DataMinMax.XCalc_YCalc)))));
            //  std delta beta vs. proportion
            chartsList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, xx, yy2, ep, dbs, dbs + " vs. " + ep, false, DataMinMax.XCalc_YCalc)))));
            //  delta deviance vs. proportion
            chartsList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, xx, yy3, ep, dd, dd + " vs. " + ep, false, DataMinMax.XCalc_YCalc)))));
            //  delta x2 vs. proportion
            chartsList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xyz, new XyzOptions(host.ChartPreferences, xx, yy4, yy1, ep, dx, dx + " (delta beta as marker size) vs. " + ep, false, DataMinMax.XCalc_YCalc)))));
            //  delta beta vs. hi
            chartsList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, hi, yy1, lv, db, db + " vs. " + lv, false, DataMinMax.XCalc_YCalc)))));
            //  delta beta std vs. hi
            chartsList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, hi, yy2, lv, dbs, dbs + " vs. " + lv, false, DataMinMax.XCalc_YCalc)))));
            //  delta deviance vs. hi
            chartsList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, hi, yy3, lv, dd, dd + " vs. " + lv, false, DataMinMax.XCalc_YCalc)))));
            //  delta x2 vs. hi
            chartsList.Add(new ParameterBag("chart", FilledParameterFactory.Output(ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, hi, yy4, lv, dx, dx + " vs. " + lv, false, DataMinMax.XCalc_YCalc)))));
            return new StepOutput(outputParameters);
        }
        ...
        public static StepOutput RptLogisticRegressionClassification(IChartPreferencesHost host, ParameterBag parameters)
        {
            MultipleLinearRegressionContext context = (MultipleLinearRegressionContext)parameters["context"].AsObject;
            int n = context.N;
            double[] fvl = context.FV;
            ...
            outputParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy0To1, new XyOptions(host.ChartPreferences, rx, ry, "1-specificity", "sensitivity", string.Empty, false, DataMinMax.XPreset_YPreset)));
            return new StepOutput(outputParameters);
        }
        ...
        public static StepOutput RptPoissonRegressionResiduals(IChartPreferencesHost host, ParameterBag parameters)
        {
            MultipleLinearRegressionContext context = (MultipleLinearRegressionContext)parameters["context"].AsObject;
            int nx = context.N;
            double[] yfit = context.FV;
            ...
            ParameterBag chartParameters = new();
            chartList.Add(chartParameters);
            chartParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, yfit, r, "Fitted Y", "Abs(Deviance residual)", "Absolute Residuals vs. Fitted Y [Poisson regression]", false, DataMinMax.XCalc_YCalc)));
            for (int i = 1; i <= P; i++)
            {
                if (i > 1 || !Intercept)
                {
                    ...
                    if (ok)
                    {
                        ...
                        chartList.Add(chartParameters);
                        chartParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, r, dr, "Predictor: " + labels[k], "Deviance residual", "Residuals vs. Predictor " + k.ToString() + " [Poisson regression]", true, DataMinMax.XCalc_YCalc)));
                    }
                }
            }
            return new StepOutput(outputParameters);
        ...
        public static StepOutput PlotProbit(IChartPreferencesHost host, ParameterBag parameters)
        {
            MultipleLinearRegressionContext context = (MultipleLinearRegressionContext)parameters["context"].AsObject;
            int model = context.M;
            double[] x = context.X1;
            ...
            ParameterBag outputParameters = new();
            outputParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Logit, new LogitOptions(host.ChartPreferences, "Proportional Response with " + Formatting.XRound(ici * 100, 1) + "% CI", model, t, sw, s1, a, b, xAxisTitle, yAxisTitle, clog), new DoubleSeries(x, xAxisTitle), new DoubleSeries(y, yAxisTitle)));
            return new StepOutput(outputParameters);
        }
