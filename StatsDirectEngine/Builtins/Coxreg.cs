...
namespace StatsDirect.Builtins
{
    public static class Coxreg
    {
        ...
        public static StepOutput RptCoxBaselineToReport(IChartPreferencesHost host, ParameterBag parameters)
        {
            return RptCoxBaseline(host.ChartPreferences, parameters, false, string.Empty, false);
        }

        public static StepOutput RptCoxBaselineToWorksheet(IChartPreferencesHost host, ParameterBag parameters)
        {
            return RptCoxBaseline(host.ChartPreferences, parameters, false, string.Empty, true);
        }

        public static StepOutput RptCoxHazardPlots(IChartPreferencesHost host, ParameterBag parameters)
        {
            ...
            for (int i = 0; i < selectedGroups.Length; i++)
            {
                if (selectedGroups[i])
                {
                    ...
                    return RptCoxBaseline(host.ChartPreferences, parameters, true, selectedGroup, false);
                }
            }
            return StepOutput.Empty();
        }

        private static StepOutput RptCoxBaseline(IChartPreferences chartPreferences, ParameterBag parameters, bool plot, string groupVar, bool createGrid)
        {
            int i;
            double watch_time;
            ...
            if (plot)
            {
                ...
                CoxPlot(chartPreferences, parameters, z, iobs, istrata, groupVar, outputParameters);
            }
            else if (createGrid)
            {
        ...
        private static void CoxPlot(IChartPreferences chartPreferences, ParameterBag parameters, CoxP[] z, int iobs, int istrata, string groupVar, ParameterBag outputParameters)
        {
            int igroups;
            int groupid = 0;
            ...
            cox1Parameters.AddOutput("chart", ChartRendererFactory.PrepForLater(chartPreferences, ChartType.CoxSurvivalOrHazard, new CoxSurvivalOrHazardOptions(chartPreferences, z, iobs, istrata, CoxPlotMode.Survival, igroups, groupid, grouped, stratified, ARR3, CDAT1, use_tic, use_marker)));
            cox1Parameters = new ParameterBag();
            chartList.Add(cox1Parameters);
            cox1Parameters.AddOutput("chart", ChartRendererFactory.PrepForLater(chartPreferences, ChartType.CoxSurvivalOrHazard, new CoxSurvivalOrHazardOptions(chartPreferences, z, iobs, istrata, CoxPlotMode.Hazard, igroups, groupid, grouped, stratified, ARR3, CDAT1, use_tic, use_marker)));
            ...
            if (grouped)
            {
                ...
                cox2Parameters.AddOutput("chart", ChartRendererFactory.PrepForLater(chartPreferences, ChartType.Cox2, new Cox2Options(chartPreferences, gn, igroups, xp, yp, CDAT1, groupid)));
            }
        }

        public static StepOutput RptCoxResiduals(IChartPreferencesHost host, ParameterBag parameters)
        {
            int istrata = 0;
            int lastStratum = 0;
            ...
            chartParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, xp, yp, "Time to event", "Deviance residual", "Deviance residuals vs. times", false, DataMinMax.XCalc_YCalc)));

            chartParameters = new ParameterBag();
            chartList.Add(chartParameters);
            chartParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(host.ChartPreferences, ChartType.Xy, new XyOptions(host.ChartPreferences, xr, yp, "Rank of time to event", "Deviance residual", "Deviance residuals vs. ranks of times", false, DataMinMax.XCalc_YCalc)));
