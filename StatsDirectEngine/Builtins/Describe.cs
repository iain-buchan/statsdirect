    ...
    public static class Describe
    {
        ...
        public static StepOutput RptTimeSeriesSummary(/* TODO: IPreferencesAndProgressBar */ ITemplateHost host, ParameterBag parameters)
        {
            ...
            for (int groupIndex = 0; groupIndex < groups.Count; groupIndex++)
            {
                ...
                ChartDefinition cd = new(host.ChartPreferences) { ChartType = ChartType.ScatterXY, ScaleParameters = new ScaleParameters { X = new AxisScaleParameters { ScaleType = ScaleType.Linear }, Y = new AxisScaleParameters { ScaleType = ScaleType.Linear } } };
                ...
                ScatterXYOptions options = new(cd.ChartPreferences, cd.XSeries, true)
                {
                ...
                for (int marker = 0; marker < options.MarkerTypes.Count; marker++)
                    options.MarkerTypes[marker] = host.ChartPreferences.MarkerTypes[ChartOptions.SeriesNumberToMarkerNumber(groupIndex)].Clone();
            ...
            {
                ChartDefinition cd = new(host.ChartPreferences) { ChartType = ChartType.Normal };
                cd.AddXSeries(points, "Area Under Curve");

                NormalOptions options = new(host.ChartPreferences)
                {
            ...
            // Line plot, one line per group with x-axis = time, y-axis = observation
            {
                ...
                ChartDefinition cd = new(host.ChartPreferences) { ChartType = ChartType.ErrorBar, ScaleParameters = new ScaleParameters { X = new AxisScaleParameters { ScaleType = ScaleType.Linear }, Y = new AxisScaleParameters { ScaleType = ScaleType.Linear } } };
                ErrorBarOptions options = new(cd.ChartPreferences)
                {
                    Series = errorSeries,
