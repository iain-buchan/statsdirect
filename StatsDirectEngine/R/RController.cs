...
namespace StatsDirect.R
{
    public static class RController
    {
        ...
        public static Process RunScriptAndQuit(IRControllerHost host, string scriptBody, out string rtfScriptBody)
        {
            string rFolder = host.MyStatsDirectRFolder;
            ...
            // We get a right mix of terminations at this point; we need environment newlines in order to match the rest of the file format and meet the requirement to be openable in platform editors.
            string repairedScriptBody = scriptBody
                .Replace("\r", string.Empty)
                .Replace("\n", Environment.NewLine);
            rtfScriptBody = (string.Format(SCRIPT_HEAD, rFolder.Replace(@"\", @"\\")) + Environment.NewLine + repairedScriptBody)
                .Replace(@"\", @"\\")
                .Replace("{", @"\{")
                .Replace("}", @"\}")
                .Replace("\n", "\n\\par ");
            ...
            while (preferredVersion is null)
            {
                if (!host.RequestRInstallation())
        ...
        internal static ParameterBag FilesToParameterBag(IRControllerHost host)
        {
            string rFolder = host.MyStatsDirectRFolder;
        ...
        public static string GetErrorText(IRControllerHost host)
        {
            string rFolder = host.MyStatsDirectRFolder;
            string errorFilePath = Path.Combine(rFolder, ERROR_FILE_NAME);
            if (!File.Exists(errorFilePath))
