...
namespace StatsDirect.Utilities
{
    public static class Formatting
    {
        ...
        public static string XRound(double amount, int places)
        {
            ...
            // a zero, including a negative zero, is 0; a value too small for the places, or beyond the 15 figures a double holds exactly
            // as a whole number, is shown in exponent form
            if (0.0 == amount)
                return "0";
            if (Math.Abs(amount) < Math.Pow(10, -places) || Math.Abs(amount) > 1e15)
            ...
            if (places >= 15)
            {
                ...
                int decimals = Math.Max(0, Math.Min(places, 15 - (Math.Abs(amount) < 1.0 ? 0 : (int)Math.Floor(Math.Log10(Math.Abs(amount))) + 1)));
                string fixedPoint = amount.ToString("F" + decimals);
                // trailing zeros are trimmed only after a decimal point: a whole number of 14 or more digits keeps its zeros
                return fixedPoint.Contains(DecimalSeparator) ? fixedPoint.TrimEnd('0').TrimEnd(DecimalSeparator.ToCharArray()) : fixedPoint;
            }
            // no thousands separators: the raw values printed beside rounded ones have none, and neither do the worksheet and the calculator
