...
namespace StatsDirect.Builtins
{
    public static class Rates
    {
        ...
        public static ParameterBag RptStdrr(ITemplateHost host, ParameterBag parameters)
        {
            ...
            chartParameters.AddOutput("chart", ChartRendererFactory.PrepForLater(ChartType.Correlation, new CorrelationOptions(k + 2, title, rkr, rkrl, rkru, rkw, pg, "Stratified rate ratio plot (direct standardization)", "rate ratio (" + Formatting.XRound(cco * 100, 1) + "% confidence interval)", Transformation.Log, false, host.Preferences.ShouldUseColour)));
