...
namespace StatsDirect.Builtins
{
    public class CoxRegression
    {
        ...
        public CoxRegression(IChartRendererFactory chartRendererFactory)
        {
        ...
        public static StepOutput RptCoxRegression(ParameterBag parameters)
        {
            ...
            int icov = 0;
            int ncov;
            if (parameters.ContainsKey("predictors") && parameters["predictors"] is not null)
            {
            ...
            if (centre)
            {
                for (int i = 1; i <= nef; i++)
                {
                    if (xd[i - 1].Groups is null || xd[i - 1].Groups.Count > 2)
                    {
                        double xbar = 0;
                        for (int r = 1; r <= rows; r++)
                            xbar += holdx[r, i];
        ...
        public StepOutput RptCoxBaselineToReport(ParameterBag parameters)
        {
            return RptCoxBaseline(parameters, false, string.Empty, false);
        }

        public StepOutput RptCoxBaselineToWorksheet(ParameterBag parameters)
        {
            return RptCoxBaseline(parameters, false, string.Empty, true);
        }

        public StepOutput RptCoxHazardPlots(ParameterBag parameters)
        {
        ...
        private StepOutput RptCoxBaseline(ParameterBag parameters, bool plot, string groupVar, bool createGrid)
        {
            int i;
            double watch_time;
        ...
        private void CoxPlot(ParameterBag parameters, CoxP[] z, int iobs, int istrata, string groupVar, ParameterBag outputParameters)
        {
            int igroups;
            int groupid = 0;
        ...
        public StepOutput RptCoxResiduals(ParameterBag parameters)
        {
            int istrata = 0;
            int lastStratum = 0;
