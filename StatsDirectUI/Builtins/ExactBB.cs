...
{
    /// <remarks>TODO: The interface between the polynomial solver and this should be far cleaner; Rachel got partway through separating it.</remarks>
    public class ExactBB : PolynomialSolver
    {
        ...
        public ExactBB(IProgressBarHost progressBarHost)
        {
            ProgressBarHost = progressBarHost;
        }

        public void Exact22K(int lowerBound, int numTables, Exact22KDataType dataType, Rec2X2[] tables, double confLevel, out double cMLE, out double upFishLim, out double loFishLim, out double upMidPLim, out double loMidPLim, out double fishP1, out double fishP2, out double midP1, out double midP2, ref bool useLogScale, out int ierr)
        {
        ...
        private void CalcPoly(Exact22KDataType dataType, int lowerBound, int numTables, Rec2X2[] tables, out int ierr)
        {
            ...
            for (int i = lowerBound + 1; i < lowerBound + numTables; i++)
            {
                if (tables[i].IsInformative)
                {
                    ...
                    if (ierr != 0)
                        return;
                    (polyDenominator, degDenominator, ierr) = MultPoly(ProgressBarHost, polyDenominator, poly2, degDenominator, deg2, (i - 1).ToString() + " of " + (numTables - 1).ToString(), UseLogScale);
                    if (ierr != 0)
                        return;
                }
            }
        ...
        public void OddsRatioCI(double cco, double a, double b, double c, double d, out double eor, out double llf, out double ulf, out bool lerr, out bool uerr)
        {
            if (a == 0 && b == 0 || c == 0 && d == 0)
            {
            ...
            else
            {
                if (a * d != 0 || b * c != 0)
                {
                    ...
                    Exact22K(0, 1, Exact22KDataType.Type1, tabl, cco, out eor, out ulf, out llf, out double _, out double _, out double _, out double _, out double _, out double _, ref useLogScale, out int _);
                }
                else
                {
        ...
        public void OddsRatioCMLE(double cco, double a, double b, double c, double d, out double eor, out double llf, out double ulf, out double llm, out double ulm, out double p1f, out double p2f, out double p1m, out double p2m, out int ierr)
        {
            eor = Constant.MISSING;
            llf = Constant.MISSING;
            ulf = Constant.MISSING;
            ...
            else
            {
                if (a * d != 0 || b * c != 0)
                {
                    ...
                    tabl[0].IsInformative = a * d != 0 || b * c != 0;
                    bool useLogScale = false;
                    Exact22K(0, 1, Exact22KDataType.Type1, tabl, cco, out eor, out ulf, out llf, out ulm, out llm, out p1f, out p2f, out p1m, out p2m, ref useLogScale, out ierr);
                }
                if ((a == 0) || (d == 0))
                {
                    eor = 0;
