...
namespace StatsDirect.Builtins
{
    ...
    { 
        ...
        { 
            return RptDescriptive( host, parameters, 1 ); 
        } 
        
        
        public static StepResult RptWeightedUnivariateSummary( ITemplateHost host, ParameterBag parameters ) 
        { 
            return RptDescriptive( host, parameters, 2 ); 
        } 
        
        
        private static StepResult RptDescriptive( ITemplateHost host, ParameterBag parameters, int index ) 
        {
            int i; int j;
            ...
            if ( index == 2 ) 
            { 
                //  Index = 2: Weighted univariate summary
                // If GIDV Then
                //     OK = GIDXW(2, 1, 200, 0, vbNullString, w, wti, nsumwt)
                // Else
                //     OK = GetCellArray(2, 1, 200, "Select data to describe", True)
                ...
                x = new double[cols + 1 /* VB to C# conversion */, maxrows + 1 /* VB to C# conversion */];
                cdx = new ColumnData[cols + 1 /* VB to C# conversion */ ]; 
                for ( i=0; i <= cols; i++ ) 
                { 
                ...
                //  If Len(wti) = 0 Then
                w = new double[cols + 1 /* VB to C# conversion */, maxrows + 1 /* VB to C# conversion */]; 
                ...
                for ( i=1; i <= weightsVariable.Length; i++ ) 
                { 
                    if ( weightsVariable.Data[i - 1] < 0.0 ) 
                    { 
                    ...
                    for ( j=0; j <= cols; j++ ) 
                    { 
                        w[ j, i ] = weightsVariable.Data[i - 1]; 
                    } 
                } 
                //  End If
            } 
            ...
            string[] titles = { "Valid data", "Missing data", "Sum", "Mean", "Variance", "Standard deviation", "Variance coefficient", "Standard error of mean", "Upper" + qxcl, "Lower" + qxcl, "Geometric mean", "Skewness", "Kurtosis", "Maximum", "Upper quartile", "Median", "Lower quartile", "Minimum", "Range", "User defined centiles", null }; 
            ...
            for ( j=0; j <= 20; j++ ) 
            { 
                fieldsList.Add( FillField( host, j, sx, cols, checked1[ j ], titles[ j ], checked1[ 19 ] ) ); 
            } 
        ...
        private static ParameterBag FillField( ITemplateHost host, int opt, Summary[] sx, int cols, bool optChecked, string optTitle, bool checked19 ) 
        { 
