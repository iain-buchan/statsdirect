...
namespace StatsDirect.Charting.Renderer
{
    class Xy0To1ChartRenderer : AbstractXYChartRenderer, IChartRenderer
    {
        ...
        ParameterBag IChartRenderer.Plot(ITemplateHost host, bool isForReturnedParametersOnly)
        {
            ...
            StartVectorPlot();
            PlotXYInternal(options.x, options.y, options.xtxt, options.ytxt, options.title, options.zPlot, options.minMaxY, 6, MarkerShape.Circle, false, PenDescriptor.Black, false, ChartAreaShape.Default, 0, 1, 0, 1);
            EndVectorPlot();
            return new ParameterBag();
        }
    }
